{   # All relative paths are related to this config file path.
    ...
    "training": {
        "dataset": {
            "path": "../data/czech_library_small_single_sentence_train.txt",  # path to training dataset
    ...
    "space": {
        # search space
        "batch": [32, 64, 128],
        "max_steps": [10000, 20000, 40000, 80000],
        "lr": (1e-5, 1e-1),  # lower and upper
        "scheduler": [None, "linear"],
        "scheduler_warmup_proportion": [0., 0.01, 0.25],
        "samples": 10,  # number of samples per one hyper tune run
    },
    "gpu": {
