{   # All relative paths are related to this config file path.
    "name": "TRAIN QUERY BASED",   # name of experiment (could be used for file naming)
    "fixed_seed": None,
    "model": {
    ...
    "training": {
        "dataset": {
            "path": "../data/train.jsonl",  # path to training dataset
        ...
        "scheduler": {
            "type": None,  # Options: None, linear, cosine, constant (see https://huggingface.co/transformers/main_classes/optimizer_schedules.html#schedules)
            "scheduler_warmup_proportion": 0.25,  # scheduler_warmup_proportion * max_steps is number of steps for warmup
        },
        "online_weighting": False    # loss will use online calculated class weights
    },
    "validation": {
        "dataset": {
        ...
        "steps": 50,  # After each X optimization steps the validation will be performed.
